...
def main():
    ...
    model.load_state_dict(torch.load(args.path_to_pth)["state_dict"])
    ...
    with torch.no_grad():
        ...
        if writer:
            ...
            for name, param in model.named_parameters():
                ...
                writer.add_histogram("val/{}".format(name), param.clone().cpu().data.numpy(), epoch)
                # writer.add_histogram("val/{}_grad".format(name), param.grad.clone().cpu().data.numpy(), epoch)
