...
parser.add_argument(
    ...
    type=int,
    default=1,
    help="sequence length of data (default: 1)",
)
parser.add_argument(
    "--seq_data_process",
    type=str,
    default="weight_sum",
    help="seq data process (weight_sum | transformer)",
)
parser.add_argument(
    "--use_admin",
    action="store_true",
    default=False,
    help="use admin-torch",
)
...
class WaypointMSELoss:
    def __init__(self, MSE_loss=torch.nn.MSELoss):
        self.loss = MSE_loss(reduction="none")
        self.weights = [
            0.1407441030399059,
            0.13352157985305926,
            0.12588535273178575,
...
class MVTMSELoss:
    def __init__(self, weight=1, MSE_loss=torch.nn.MSELoss):
        self.loss = MSE_loss()
        self.weight = weight
    ...
    def __call__(self, output, target):
        ...
        if target_2.numel() == 0:
            ...
            loss3_prob_1 = self.loss(target_2, output_2)
        output_3 = output[target_0_mask][:][:, 6]
        target_3 = target[target_0_mask][:][:, 6]
        if target_3.numel() == 0:
            loss3_prob_0 = 0
        else:
            loss3_prob_0 = self.loss(target_3, output_3)
        loss_3 = 0.05 * loss3_prob_0 + 0.95 * loss3_prob_1
...
def main():
    ...
    model = create_model(
        ...
        seq_n=args.seq_n,
        seq_data_process=args.seq_data_process,
    ...
    if args.pretrained:
        pretrained_dict = torch.load(args.initial_checkpoint)["state_dict"]
...
def train_one_epoch(
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if isinstance(input, (tuple, list)):
            ...
            if 'seq_n' in input.keys():
                for k, v in input.items():
                    if isinstance(v, list):
                        input[k] = torch.stack(input[k], dim=1)
        ...
        with amp_autocast():
            ...
            # ## addition train for turning 

            # wp_ = target[1].cpu()
            # aim_ = torch.add(wp_[:, 1, :],  wp_[:, 0, :]).numpy()
            # aim_[:, 1] *= -1
            # angle_ = np.degrees(np.pi / 2 - np.arctan2(aim_[:, 1], aim_[:, 0]))
            # temp_ = torch.tensor(np.abs(angle_) > 15, dtype=torch.bool)
            
            # for i in range(len(target)):
            #     target[i] = target[i][temp_, ...]

            output = model(input)

            # output = list(output)
            # for i in range(len(output)):
            #     output[i] = output[i][temp_, ...]

            
            
            loss_traffic, loss_velocity = loss_fns["traffic"](output[-1], target[4])
            ...
            loss_brake = loss_fns["cls"](output[0], target[7])
        ...
        if last_batch or batch_idx % args.log_interval == 0:
            ...
            if args.distributed:
                ...
                reduced_loss_brake = reduce_tensor(
                    loss_brake.data, args.world_size
                )
                losses_brake.update(reduced_loss_brake.item(), batch_size)
                ...
                if writer and args.local_rank == 0:
                    ...
                    writer.add_scalar(
                        "train/loss_brake", reduced_loss_brake.item(), num_updates
                    )
                    ...
                    writer.add_image(
                        "train/pred_traffic",
                        torch.clip(output[-1][0], 0, 1).view(1, 20, 20, 7)[:, :, :, 0],
                        num_updates,
                    )
                    writer.add_image(
                        ...
                        torch.clip(
                            torch.tensor(
                                render(
                                    output[-1][0].view(20, 20, 7).detach().cpu().numpy()
                                )[:100, 40:140]
                            ),
                            0,
                            255,
                    ...
                    #     writer.add_histogram("train/{}".format(name), param.clone().cpu().data.numpy(), epoch)
                    #     writer.add_histogram("train/{}_grad".format(name), param.grad.clone().cpu().data.numpy(), epoch)
            ...
            if args.local_rank == 0:
                _logger.info(
                    ...
                    "Loss(brake): {loss_brake.val:>9.6f} ({loss_brake.avg:>6.4f})  "
        ...
        #     for name, param in model.named_parameters():
        #         writer.add_histogram("train/{}".format(name), param.clone().cpu().data.numpy(), epoch)
        #         writer.add_histogram("train_grad/{}".format(name), param.grad.clone().cpu().data.numpy(), epoch)
...
def validate(
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            brake_error = accuracy(output[0], target[7])[0]
            ...
            if args.distributed:
                ...
                reduced_brake_error = reduce_tensor(brake_error, args.world_size)
            ...
            brake_errorm.update(reduced_brake_error.item(), batch_size)
            ...
            if args.local_rank == 0 and (
                ...
                _logger.info(
                    ...
                    "Acc(brake): {brake_errorm.val:>9.6f} ({brake_errorm.avg:>6.4f})  "
        ...
        if writer:
            ...
            writer.add_scalar("val/loss_brake", losses_brake.avg, epoch)
            writer.add_scalar(
            ...
            writer.add_scalar("val/acc_brake", brake_errorm.avg, epoch)
            writer.add_scalar(
