...
from team_code.demo_controller import InterfuserController
...
class DemoNetAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        self.rgb_left_transform = create_carla_rgb_transform(224)
        self.rgb_right_transform = create_carla_rgb_transform(224)
        self.rgb_center_transform = create_carla_rgb_transform(224, need_scale=False)
        ...
        if self.ensemble:
            ...
            self.net = create_model(
                self.config.model,
                use_admin=self.config.use_admin,
                only_encoder=self.config.only_encoder,
                )
            path_to_model_file = self.config.model_path
            print('load model: %s' % path_to_model_file)
            self.net.load_state_dict(torch.load(path_to_model_file)["state_dict"], False)
            self.net.cuda()
            self.net.eval()
        ...
        if SAVE_PATH is not None:
            ...
            if SAVE_PATH is not None:
                ...
                string += "_".join(
                    map(
                        lambda x: "%02d" % x,
                        (now_time.month, now_time.day, now_time.hour, now_time.minute, now_time.second),
                    )
                )
                
                print("save dir : {}".format(string))
                
                self.save_dir = string
    ...
    def run_step(self, input_data, timestamp):
        ...
        if self.ensemble:
            ...
            with torch.no_grad():
                (
                    ...
                    stop_sign,
                    # bev_feature,
                ) = self.net.forward_val(input_data)
        ...
        brake = self.softmax(brake).detach().cpu().numpy().reshape(-1)[1]
        traffic_light_state = (
        ...
        steer, throttle, _ = self.controller.run_step(
            velocity,
            pred_waypoints,
            is_junction,
            traffic_light_state,
        ...
        # print("throttle: {}, steer: {}".format(control.throttle, control.steer))
        ...
        #         safe_index = i
        #         break
        wp = render_waypoints(pred_waypoints[:safe_index], pixels_per_meter=20, color=(0, 255, 0))[:400, 160:560]
