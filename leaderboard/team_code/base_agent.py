...
class BaseAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        print(self.config)
        print("save skip frames : {}".format(self.save_skip_frames))
        self.rgb_only = self.config.get("rgb_only", True)

        if self.weather_id == 'random':
            self.weather_id = random.randint(0, len(WEATHERS_IDS) - 1)
        else:
            self.weather_id = 0

        print("weather : {}".format(WEATHERS_IDS[self.weather_id]))
        ...
        if SAVE_PATH is not None:
            ...
            if self.weather_id is None:
                ...
                self.save_weather = WEATHERS_IDS[self.weather_id]
