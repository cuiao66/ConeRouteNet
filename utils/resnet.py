...
class ResNet_Bottleneck_OS16(nn.Module):
    def __init__(self, num_layers, project_dir):
        super(ResNet_Bottleneck_OS16, self).__init__()

        if num_layers == 50:
            resnet = models.resnet50()
            # load pretrained model:
            resnet.load_state_dict(torch.load(os.path.join(project_dir, "deeplabv3/pretrained_models/resnet/resnet50-19c8e357.pth")))
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load(os.path.join(project_dir, "deeplabv3/pretrained_models/resnet/resnet101-5d3b4d8f.pth")))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load(os.path.join(project_dir, "deeplabv3/pretrained_models/resnet/resnet152-b121ed2d.pth")))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
class ResNet_BasicBlock_OS16(nn.Module):
    def __init__(self, num_layers, project_dir):
        ...
        if num_layers == 18:
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load(os.path.join(project_dir, "deeplabv3/pretrained_models/resnet/resnet18-5c106cde.pth")))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
            ...
            # load pretrained model:
            resnet.load_state_dict(torch.load(os.path.join(project_dir, "deeplabv3/pretrained_models/resnet/resnet34-333f7ec4.pth")))
            # remove fully connected layer, avg pool and layer5:
            self.resnet = nn.Sequential(*list(resnet.children())[:-3])
...
def ResNet18_OS16(project_dir):
    return ResNet_BasicBlock_OS16(project_dir=project_dir, num_layers=18)

def ResNet34_OS16(project_dir):
    return ResNet_BasicBlock_OS16(project_dir=project_dir, num_layers=34)

def ResNet50_OS16(project_dir):
    return ResNet_Bottleneck_OS16(project_dir=project_dir, num_layers=50)

def ResNet101_OS16(project_dir):
    return ResNet_Bottleneck_OS16(project_dir=project_dir, num_layers=101)

def ResNet152_OS16(project_dir):
    return ResNet_Bottleneck_OS16(project_dir=project_dir, num_layers=152)

def ResNet18_OS8(project_dir):
    return ResNet_BasicBlock_OS8(project_dir=project_dir, num_layers=18)

def ResNet34_OS8(project_dir):
    return ResNet_BasicBlock_OS8(project_dir=project_dir, num_layers=34)
